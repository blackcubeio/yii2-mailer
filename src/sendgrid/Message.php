...
namespace blackcube\mailer\sendgrid;
...
class Message extends BaseMessage
{
    ...
    {
        $fromMail = null;
        $email = array_key_first($this->from);
        $name = $this->from[$email] ?? null;
    ...
    public function getReplyTo()
    {
        ...
        if (is_array($this->replyTo) === true) {
            $email = array_key_first($this->replyTo);
            $name = $this->replyTo[$email] ?? null;
            if (is_numeric($email) === true) {
                $replyTo = $name;
            } else {
                $replyTo = $email;
